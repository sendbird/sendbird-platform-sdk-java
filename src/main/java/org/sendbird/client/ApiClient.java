...
import org.sendbird.client.auth.Authentication;
import org.sendbird.client.auth.HttpBasicAuth;
import org.sendbird.client.auth.HttpBearerAuth;
import org.sendbird.client.auth.ApiKeyAuth;
...
public class ApiClient extends JavaTimeFormatter {
  ...
  protected String basePath = "https://api-APP_ID.sendbird.com";
  ...
  protected List<ServerConfiguration> servers = new ArrayList<ServerConfiguration>(Arrays.asList(
    new ServerConfiguration(
      ...
      new HashMap<String, ServerVariable>() {{
        put("app_id", new ServerVariable(
          "Replace with the APP_ID for your app in the Sendbird dashboard",
          "APP_ID",
          new HashSet<String>(
          )
        ));
    ...
    if (contentType.startsWith("multipart/form-data")) {
      ...
      if (isBodyNullable) { // payload is nullable
        if (obj instanceof String) {
          ...
          try {
            String jsonString = obj == null ? "null" : json.getMapper().writeValueAsString(obj);
            entity = Entity.entity(jsonString, contentType);
          } catch (Exception e) {
            throw new ApiException("Failed to serialize object: " + e.toString());
          }
        }
      } else {
        if (obj instanceof String) {
          ...
          try {
            String jsonString = obj == null ? "" : json.getMapper().writeValueAsString(obj);
            entity = Entity.entity(jsonString, contentType);
          } catch (Exception e) {
            throw new ApiException("Failed to serialize object: " + e.toString());
          }
